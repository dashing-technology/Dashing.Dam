...
class DropboxRESTClient
{
    ...
    protected $curlDefaults = array(
        ...
        // INTEGERS
        CURLOPT_TIMEOUT        => 18000,       // cURL timeout
        CURLOPT_CONNECTTIMEOUT => 18000,       // Connection timeout

        // STRINGS
        CURLOPT_ENCODING       => "",       // "identity", "deflate", and "gzip"
        CURLOPT_USERAGENT      => "Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.7"
    );
    ...
    public function request($url, $method = "GET", $headers = array(), $params = array(), $body = null, $raw = false) {
        ...
        return array("code" => $status, "body" => $body, "headers" => $response_headers,"info"=>$info,"response"=>$response);
    }
}
