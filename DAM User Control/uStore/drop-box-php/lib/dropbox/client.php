...
class DropboxClient
{
	...
	public function metadataSearch($query,$path, $fileLimit = 10000, $revision = null, $includeDeleted = false) {
        // Prepare argument list
        $args = array(
            "file_limit" => $fileLimit,
			 "query" => $query,
        ...
        $path = str_replace(" ","%20",$path);
		$path = str_replace("'","%27",$path);
        $response = $this->Session->fetch("GET", $this->dropboxAPIURL, "/search/" . $path, $args);
	...
	public function addFolder($path) 
    {
        ...
        $response = $this->Session->fetch("GET", $this->dropboxAPIURL, "/fileops/create_folder/", $args);
        return $response["body"];
    }
	...
	public function renameFile($path,$newPath) 
    {
        ...
        $response = $this->Session->fetch("GET", $this->dropboxAPIURL, "/fileops/move/", $args);
        return $response["body"];
    }
	...
	public function removeFile($path) 
    {
        ...
        $response = $this->Session->fetch("GET", $this->dropboxAPIURL, "/fileops/delete/", $args);
        return $response["body"];
    }
    ...
    public function thumbnail($path) {
        ...
        $path = str_replace(" ","%20",$path);
		$path = str_replace("'","%27",$path);
        $response = $this->Session->fetch("GET", $this->dropboxContentAPIURL, "/thumbnails/" . $path, $args,true);
	...
	public function preview($path,$rev) {
        // Prepare argument list
        $args = array("rev"=>$rev);
        ...
        $path = str_replace(" ","%20",$path);
		$path = str_replace("'","%27",$path);
        $response = $this->Session->fetch("GET", $this->dropboxContentAPIURL, "/previews/auto/" . $path, $args,true);
	...
	public function oauth1to2() 
    {
        ...
        $path = str_replace(" ","%20",$path);
		$path = str_replace("'","%27",$path);
        $response = $this->Session->fetch("POST", $this->dropboxAPIURL, "/oauth2/token_from_oauth1", $args,true);
    ...
    public function getFile($path, $outFile = null, $revision = null) {
		...
		{
			throw new Exception("Data Missing: ".var_export($response,true));
		}
